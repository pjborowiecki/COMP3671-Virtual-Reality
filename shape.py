...
class Triangle:
	...
	def contains_point(self, point):
		...
		if (a >= 0 and b >= 0 and c >= 0):
			...
			red = int(a*self.p0.color.r() + b*self.p1.color.r() + c*self.p2.color.r())
			green = int(a*self.p0.color.g() + b*self.p1.color.g() + c*self.p2.color.g())
			blue = int(a*self.p0.color.b() + b*self.p1.color.b() + c*self.p2.color.b())
			alpha = int(a*self.p0.color.a() + b*self.p1.color.a() + c*self.p2.color.a())

			# Also interpolate the z-value of this point
			zValue = a*self.p0.z + b*self.p1.z + c*self.p2.z
			return True, Color(red, green, blue, alpha), zValue

		else:
			return False, None, None

	def draw(self, surface, zBuffer):
		...
		xmax = min(max(self.p0.x, self.p1.x, self.p2.x), width - 1)
		ymin = max(min(self.p0.y, self.p1.y, self.p2.y), 0)
		ymax = min(max(self.p0.y, self.p1.y, self.p2.y), height - 1)
		...
		for x in range(xmin, xmax + 1):
			for y in range(ymin, ymax + 1):
				...
				if point_in_triangle:
					#print("color=", color.getTuple() if color is not None else None, "zval=", zValue, "zbuff=", zBuffer[y*width + x])
					# Check z-buffer to determine whether to draw this pixel

					if zBuffer[y*width + x] > zValue:
						#print()
						zBuffer[y*width + x] = zValue
						#print(color.getTuple())
						surface.set_at((x,y), color.getTuple())
